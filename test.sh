...
$PY2_32_MINGW -c 'from memmod.tests.test_memmod import StrQueueTest;import unittest; unittest.main()' > /dev/null
$PIP2_32_MINGW uninstall -y memmod-$MEMMOD_VER-cp27-cp27m-mingw.whl &> /dev/null
...
$PY3_32_MINGW -c 'from memmod.tests.test_memmod import StrQueueTest;import unittest; unittest.main()' > /dev/null
$PIP3_32_MINGW uninstall -y memmod-$MEMMOD_VER-cp37-cp37m-mingw.whl &> /dev/null
...
$PY2_64_MINGW -c 'from memmod.tests.test_memmod import StrQueueTest;import unittest; unittest.main()' > /dev/null
$PIP2_64_MINGW uninstall -y memmod-$MEMMOD_VER-cp27-cp27m-mingw.whl &> /dev/null
...
$PY3_64_MINGW -c 'from memmod.tests.test_memmod import StrQueueTest;import unittest; unittest.main()' > /dev/null
$PIP3_64_MINGW uninstall -y memmod-$MEMMOD_VER-cp37-cp37m-mingw.whl &> /dev/null
...
python.exe -c 'from memmod.tests.test_memmod import StrQueueTest;import unittest; unittest.main()' > /dev/null
python.exe -m pip uninstall -y memmod-$MEMMOD_VER-cp27-cp27m-win32.whl &> /dev/null
PATH=$OLDPATH

OLDPATH=$PATH
PATH=$PYDIR3_32_WIN:$PYDIR3_32_WIN/Scripts:$PATH
echo "[*] Testing python3 lib for win32"
python.exe -m pip uninstall -y memmod-$MEMMOD_VER-cp36-cp36m-win32.whl &> /dev/null
python.exe -m pip install      memmod-$MEMMOD_VER-cp36-cp36m-win32.whl > /dev/null
python.exe -c 'from memmod.tests.test_memmod import StrQueueTest;import unittest; unittest.main()' > /dev/null
python.exe -m pip uninstall -y memmod-$MEMMOD_VER-cp36-cp36m-win32.whl &> /dev/null
PATH=$OLDPATH
...
echo "[*] Testing python2 lib for x64"
python.exe -m pip uninstall -y memmod-$MEMMOD_VER-cp27-cp27m-win_amd64.whl &> /dev/null
python.exe -m pip install      memmod-$MEMMOD_VER-cp27-cp27m-win_amd64.whl > /dev/null
python.exe -c 'from memmod.tests.test_memmod import StrQueueTest;import unittest; unittest.main()' > /dev/null
python.exe -m pip uninstall -y memmod-$MEMMOD_VER-cp27-cp27m-win_amd64.whl &> /dev/null
PATH=$OLDPATH
